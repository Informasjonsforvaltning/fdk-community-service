<ul component="category" class="topic-list" itemscope itemtype="http://www.schema.org/ItemList" data-nextstart="{nextStart}" data-set="{set}">
	...
	<li component="category/topic" class="row clearfix category-item {function.generateTopicClass}" <!-- IMPORT partials/data/category.tpl -->>
      ...
      <span class="topic-info">
				...
				<small class="hidden-xs topic-author-time">
          <span class="timeago" title="{topics.timestampISO}"></span>
          <a href="<!-- IF topics.user.userslug -->{config.relative_path}/user/{topics.user.userslug}<!-- ELSE -->#<!-- ENDIF topics.user.userslug -->">{buildAvatar(../user, "24", true, "not-responsive")}</a>
		...
		<div class="col-md-3 col-sm-3 teaser hidden-xs" component="topic/teaser">
			<div class="card background-link-container" style="border-color: {topics.category.bgColor}">
				...
				<p>
					...
					<a class="user-page-link teaser-poster" href="{config.relative_path}/topic/{topics.slug}/{topics.teaser.index}">
						<span class="teaser-poster">{topics.teaser.user.displayname}</span>
					</a>
            <span class="timeago" title="{topics.teaser.timestampISO}"></span>:
				</p>
