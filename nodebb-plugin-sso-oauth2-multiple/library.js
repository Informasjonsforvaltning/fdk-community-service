...
OAuth.loadStrategies = async (strategies) => {
	const passportOAuth2 = require('passport-oauth').OAuth2Strategy;
	...
	}, async (req, _token, _refreshToken, params, profile, done) => {
...
OAuth.setLogoutCookie = ({res, logoutUrl}) => {
	const cookie = meta.configs.cookie.get();
	const ttl = meta.getSessionTTLSeconds() * 1000;
	cookie.maxAge = ttl;

	res.cookie('OAUTH2-MULTIPLE-LOGOUT', logoutUrl, cookie);
}