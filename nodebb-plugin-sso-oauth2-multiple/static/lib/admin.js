...
function handleAutoDiscovery() {
	...
	domainEl.addEventListener('change', async () => {
		try {
			...
			const { authorization_endpoint, token_endpoint, end_session_endpoint, userinfo_endpoint } = await get(`/plugins/oauth2-multiple/discover?domain=${encodeURIComponent(domainEl.value)}`);
			...
			if (end_session_endpoint) {
				modalEl.querySelector('#logoutUrl').value = end_session_endpoint;
			}
