...
echo "*/15 * * * * TEST_MODE=$TEST_MODE TEST_EMAIL=$TEST_EMAIL API_TOKEN=$API_TOKEN API_TOKEN_WRITE=$API_TOKEN_WRITE TOKEN_UID=$TOKEN_UID BASE_URL='https://community.staging.fellesdatakatalog.digdir.no' /usr/local/bin/user-retention.sh >> /usr/src/app/files/log/cron.log 2>&1
